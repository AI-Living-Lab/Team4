...
#     - GPU 4→1 (0번), per_device_bs 1, grad_accum 2→8  => eff batch 8 유지
#       ※ 2-GPU 는 rank 간 gradient-reduce 불일치로 NCCL 교착(이질 데이터: audio
#         2000/3000·source_id 1000/3000)이 확인돼 1-GPU 로 회피. 원본 train_v8 은
#         unav 단독(동질)이라 안 터졌음. 1-GPU 는 rank-reduce 자체가 없어 정상.
#     - ordinal_enabled False (지시). 1-GPU 에선 ordinal 켜도 무방(드라이런 확인).
...
OUTPUT_DIR=/home/team404/workspace/checkpoints/sft/salmonn2plus_v8_unpucha_sft
RUN_NAME=salmonn2plus_v8_unpucha_sft
mkdir -p "$OUTPUT_DIR"
