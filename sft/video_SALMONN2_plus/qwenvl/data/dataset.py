...
try:
    ...
    # torchcodec이 PyTorch ABI/FFmpeg(libavutil)과 안 맞아 로드 실패(RuntimeError 등) 시
    # VideoDecoder 경로는 decord로 자동 fallback, AudioDecoder는 sf/soxr로 대체 → None 무해.
...
# ============================================================================
# TTI (Time-Token Interleaving) helpers
# ============================================================================
...
def make_time_marker_string(sec_start: float, tti_time_format: str,
    ...
    # tti_time_format 에 따라 입력 측 시간 마커 문자열을 생성. 출력 측(GT)은 무관.
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer, data_args):
        ...
        # TTI (Time-Token Interleaving) params cache.
        self.tti_time_format = getattr(data_args, "tti_time_format", "off")
        self.tti_marker_token_len = _TIME_MARKER_TOKEN_LEN.get(self.tti_time_format, 0) or None
        self.tti_time_token_id_range = None
        if self.tti_time_format in ("special_token", "from_to"):
            try:
                t0 = tokenizer.convert_tokens_to_ids("<t0>")
                t9 = tokenizer.convert_tokens_to_ids("<t9>")
                tdot = tokenizer.convert_tokens_to_ids("<tdot>")
                lo = min(t0, t9, tdot)
                hi = max(t0, t9, tdot)
                self.tti_time_token_id_range = (int(lo), int(hi))
            except Exception:
                self.tti_time_token_id_range = None
        if self.tti_time_format != "off":
            rank0_print(
                f"[TTI] mode={self.tti_time_format} marker_len={self.tti_marker_token_len} "
                f"time_token_id_range={self.tti_time_token_id_range}"
            )
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            data_dict = preprocess_qwen_2_visual(
                ...
                tti_time_format=getattr(self.data_args, "tti_time_format", "off"),
            )
            position_ids, _ = self.get_rope_index(
                ...
                second_per_grid_ts=second_per_grid_ts if second_per_grid_ts else None,
                audio_lengths=audio_lengths if audio_lengths else None,
                time_token_id_range=self.tti_time_token_id_range,
                time_marker_token_len=self.tti_marker_token_len,
            )
