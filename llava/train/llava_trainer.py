...
class LLaVATrainer(Trainer):
    ...
    def training_step(self, model, inputs):
        ...
        def _is_rank0():
            ...
            return getattr(self.args, "local_rank", -1) in (-1, 0)

        def _unwrap(m):
            """
            unwrap DDP / FSDP / Accelerate wrappers if present
            - DDP: m.module
            - Accelerate: accelerator.unwrap_model(m) (if available)
            """
            # accelerate unwrap (best-effort)
            try:
                if hasattr(self, "accelerator") and self.accelerator is not None:
                    m2 = self.accelerator.unwrap_model(m)
                    if m2 is not None:
                        m = m2
            except Exception:
                pass
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        # HF standard: .../checkpoint-<global_step>/
    ...
    def prediction_step(self, model, inputs, prediction_loss_only=False, ignore_keys=None, do_sample=False):
        ...
        if "spectrogram" in inputs and inputs["spectrogram"] is not None:
    ...
    def prediction_loop(self, dataloader, do_sample=False, prediction_loss_only=False):
        ...
        if dist.get_rank() == 0:
            for inputs in tqdm(dataloader):
                ...
                loss, preds, refs = self.prediction_step(model, inputs)