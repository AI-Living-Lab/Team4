...
def add_time_tokens_and_init(tokenizer, model, num_bins: int):
    ...
    with torch.no_grad():
        # bfloat16에서 151647개 벡터의 누적 합산은 overflow 위험 → float32로 계산 후 캐스팅
        in_avg = in_embed[:-num_added].float().mean(dim=0, keepdim=True).to(in_embed.dtype)
        in_embed[-num_added:] = in_avg
        if out_embed is not None:
            out_avg = out_embed[:-num_added].float().mean(dim=0, keepdim=True).to(out_embed.dtype)
...
def train():
    ...
    if training_args.lora_enable:
        ...
        # 3) if time-token SFT: lm_head + embedding 전체를 학습
        if model_args.add_time_token:
            unfreeze_embeddings_and_lm_head(model)
            # ✅ SFT에서는 only_time_tokens=False:
            #    lm_head 전체 row를 업데이트해야 time token이 문맥에 맞게 출력됨
            #    only_time_tokens=True는 token embedding 초기화 단계에만 적합
            apply_time_token_grad_mask(
                model,
                tokenizer,
                num_bins=model_args.temporal_num_bins,
                only_time_tokens=False,  # ← True에서 False로 변경
            )
        ...
        # ✅ 추가: non-LoRA에서도 add_time_token이면 lm_head + embedding unfreeze
        if model_args.add_time_token:
            unfreeze_embeddings_and_lm_head(model)
            apply_time_token_grad_mask(
                model,
        ...
        if rank == 0:
            ...
            if total_valid == 0:
                raise RuntimeError(
                    ...
                    "gpt.value의 time token 문자열이 tokenizer에 의해 올바르게 단일 토큰으로 "
                    "인코딩되지 않거나, conversation preprocess에서 response 구간 계산이 "
                    "잘못된 것입니다. dataset/collator 코드의 label 마스킹 로직을 확인하세요."
                )
            else:
                avg_valid_ratio = total_valid / total_tokens
                print(f"[LABEL SANITY] valid ratio: {avg_valid_ratio:.3f} ✅")
            print("=========================================\n")
