...
if is_flash_attn_2_available():
    ...
    # flash_attn.ops는 환경에 따라 없을 수 있으므로 안전하게 처리
    try:
        from flash_attn.ops.rms_norm import rms_norm  # optional extension
        faster_llama_rmsnorm = rms_norm
    except Exception:
        # fallback RMSNorm (no flash_attn.ops needed)
        def faster_llama_rmsnorm(x, weight, eps):
...
# ✅ 항상 정의되게 보장 (flash-attn 없으면 False)
_flash_supports_window_size = globals().get("_flash_supports_window_size", False)
