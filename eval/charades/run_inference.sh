#!/usr/bin/env bash
# ============================================================
# Charades-STA inference with vS2 + PU-VALOR LoRA checkpoint
# ============================================================
set -eo pipefail
...
# Try kept-checkpoint first, then checkpoint
LORA_PATH=/data0/aix23102/checkpoints_open_aligner/vs2_stage3_puvalor/kept-checkpoint-${CKPT}
if [ ! -d "$LORA_PATH" ]; then
    LORA_PATH=/data0/aix23102/checkpoints_open_aligner/vs2_stage3_puvalor/checkpoint-${CKPT}
fi

TEST_JSON=$BASE/eval/charades/charades_sta_test.json
TEST_OUT=$BASE/eval/charades/results/charades_test_${CKPT}

echo "=== Charades-STA Inference ==="
echo "  Checkpoint: $LORA_PATH"
