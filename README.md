...
1. **LLaVA-OneVision** — [huggingface](https://huggingface.co/lmms-lab/llava-onevision-qwen2-7b-ov)
2. **video-SALMONN-2** — [huggingface](https://huggingface.co/tsinghua-ee/video-SALMONN-2)

### 🔧 Environment Variables

`paths.env.example` 를 복사하여 `paths.env` 를 만들고 아래 값을 채웁니다:
- `BASE_DIR` — 이 레포 루트
- `CKPT_DIR` — 체크포인트 루트 (`Team4/checkpoints/`)
- `JSON_DIR` — 학습/평가 JSON 디렉토리 (`Team4/data/`)
...
│   │   └── add_time_tokens_salmonn2plus.py
│   └── gdpo/                                  # GDPO 학습 코드
└── eval/
...
**인증**: `huggingface-cli login` 또는 `HF_TOKEN=hf_xxx` 환경변수.
