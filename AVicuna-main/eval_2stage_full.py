...
with open('data/annotations/unav100_annotations.json') as f:
    gt_data = json.load(f)
with open('output/label_pred_2stage_full.json') as f:
...
print('=== 2-Stage Full (2167 videos) ===')
for t in thresholds:
    print(f'tIoU {t:.1f}: {results[t]:.2f}')
avg = np.mean([results[t] for t in thresholds])
print(f'Avg [0.1:0.9]: {avg:.2f} (paper: 60.3)')
print(f'\nComparison:')
print(f'  Exp A (describe 2167):     14.97')
print(f'  Exp C (100-label+NO 10):   31.65')
print(f'  Exp E (2-stage full):      {avg:.2f}')
