...
#  Prompt
# ================================================================== #
PROMPT = '<video>\nCould you pinpoint when {query}? Please provide the start and end time in seconds.'
...
def main():
    ...
    for s in tqdm(samples, desc="Evaluating"):
        ...
        # duration을 decord로 읽기
        try:
            from decord import VideoReader
            vr = VideoReader(f"/workspace/datasets/charades_sta/videos/{vid}.mp4")
            s["duration"] = len(vr) / vr.get_avg_fps()
        except:
            pass
