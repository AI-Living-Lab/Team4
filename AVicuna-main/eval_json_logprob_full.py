...
with open('data/annotations/unav100_annotations.json') as f:
    gt_data = json.load(f)
with open('output/json_logprob_full.json') as f:
...
print('=== Exp F-logprob: JSON + logprob confidence ===')
...
print(f'\nComparison:')
print(f'  Exp A (describe + CLAP):         14.97')
print(f'  Exp F (JSON + score=1.0):        15.35')
print(f'  Exp F (JSON + CLAP):             15.50')
print(f'  Exp F (JSON + logprob):          {avg_all:.2f}')
