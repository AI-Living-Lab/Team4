#!/bin/bash
# ============================================================
# v5 학습 후 UnAV-100 test mIoU 평가
#   Usage:  eval_v5_unav.sh <OUTPUT_DIR> <TOK> <NAME>
...
#   3) UnAV test (3,455 samples) 를 N-shard 로 분할 → 각 GPU 병렬 inference
#   4) shard 결과 재조립 → eval_miou_v5.py 로 mIoU/R@k 계산
# ============================================================
set -eo pipefail
...
TEST_JSON=$DATA_DIR/unav_test_t2_tok${TOK}.json
TEST_JSON_RAW=$DATA_DIR/unav_test_t2.json   # gt_segments 가 필요한 placeholder 버전
...
RESULTS_BASE=$OUTPUT_DIR/eval_unav_test_$(basename $LATEST_CKPT)
