#!/bin/bash
# ============================================================
# v5_A: PU-VALOR only  /  4+1 tokenization  /  NO Ordinal
#   v4 대비 변경 점:
#     - 데이터: puvalor_train_v5_tok4_1.json (90,972, T1+T2+T3)
#     - embed_lr: 8e-4 → 1e-4
#     - lm_head_lr: 8e-4 → 1e-4
#     - GPU: 6,7 (2 GPUs, effective_batch=8 via grad_accum=4)
#   토큰화/ordinal 은 v4 그대로 (4+1, ordinal off) — baseline.
# ============================================================
set -eo pipefail
...
DATASET=$DATA_DIR/puvalor_train_v5_tok4_1.json
EVAL_DATASET=$DATA_DIR/puvalor_val_v5_sub500_tok4_1.json
OUTPUT_DIR=/data0/aix23102/checkpoints_open_aligner/salmonn2plus_v5_a
RUN_NAME=salmonn2plus_v5_a
mkdir -p "$OUTPUT_DIR"
...
# ---- After training: PU-VALOR test mIoU (T1+T2) ----
echo "[A] training done; running PU-VALOR test eval"
