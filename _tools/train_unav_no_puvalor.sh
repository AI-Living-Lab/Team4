#!/bin/bash
# ============================================================
# [C] High-res + LoRA(q/k/v) + lm_head + embed (NO PU-VALOR)
#   - Base: raw video-SALMONN2+ 7B (time_tokens only)
#   - UnAV-100 0.2ep, GPU 4,5
#   - Purpose: isolate PU-VALOR pretraining effect
# ============================================================
set -eo pipefail
...
OUTPUT_DIR=/data0/aix23102/checkpoints_open_aligner/ablation_C_no_puvalor_highres
RUN_NAME=ablation_C_no_puvalor

# UnAV-100: 10358 samples, effective_batch=4 (bs=1, accum=2, gpu=2)
# 0.2ep = 518 steps, save at 0.1ep = 259 steps
mkdir -p "$OUTPUT_DIR"
