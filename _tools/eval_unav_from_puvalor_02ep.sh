...
RESULTS_BASE=/home/aix23102/audiolm/vS2_eunji/eval/results/unav_from_puvalor_lowres_0.2ep

# Wait for training to finish (all 10 checkpoints)
echo "[WAITER] $(date) Waiting for UnAV training to finish (10 checkpoints)..."
while [ "$(ls -d "$UNAV_DIR"/checkpoint-* 2>/dev/null | wc -l)" -lt 10 ]; do
    sleep 120
done
# Extra safety: training process must be gone
while pgrep -f "train_qwen.*unav_from_puvalor_lowres" > /dev/null 2>&1; do
    sleep 60
done
echo "[WAITER] $(date) UnAV training finished!"
sleep 30
...
CKPTS=($(ls -d "$UNAV_DIR"/checkpoint-* 2>/dev/null | sort -V))
STEPS_PER_01EP=259
NAMES=()
for ckpt in "${CKPTS[@]}"; do
    ...
    if [ "$ep_x10" -eq 10 ]; then
        ...
        NAMES+=("unav_0.${ep_x10}ep")
    fi
done

echo "Models to eval: ${#NAMES[@]}"

# Run up to 4 parallel (GPU 4,5,6,7)
i=0
while [ $i -lt ${#NAMES[@]} ]; do
    PIDS=()
...
echo ""
echo "════════════════════════════════════════════════════════════════"
echo "  UnAV from low-res PU-VALOR 0.2ep (1 epoch, 10 checkpoints)"
echo "════════════════════════════════════════════════════════════════"
