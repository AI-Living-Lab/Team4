#!/bin/bash
# ============================================================
# salmonn2plus_puvalor_v4 평가 (PU-VALOR test, format-hinted prompts)
#   - GPU 4,5 로 2-shard 병렬 inference
#   - PU-VALOR GOUT (puvalor_test_gout_hinted.json, 2528) 만 평가
#     (각 human turn 끝에 답변 format 예시 1줄 추가된 버전)
...
TEST_JSON=/home/aix23102/audiolm/vS2_eunji/data/puvalor_test_gout_hinted.json
...
RESULTS_BASE=/home/aix23102/audiolm/vS2_eunji/eval/results/puvalor_v4_$(basename $CKPT)
SHARD_DIR=$RESULTS_BASE/shards
mkdir -p "$RESULTS_BASE" "$SHARD_DIR"

# ---- (1) Merge LoRA → base if MERGED_MODEL doesn't exist ----
# attn=eager 로 변경: merge 단계는 forward 안 하므로 flash_attention_2 불필요.
# CUDA context 도 안 만들기 위해 CUDA_VISIBLE_DEVICES="" 로 GPU 노출 차단.
...
# [4/5] Load trained audio.q_tokens. PEFT cannot wrap nn.Parameter, so q_tokens
# is missing from adapter_model.safetensors. Try the new sidecar file first,
# fall back to DeepSpeed mp_rank_00_model_states.pt for legacy checkpoints,
# else warn that BASE q_tokens will be used.
qt_loaded = False
qt_sidecar = os.path.join(LORA, "audio_q_tokens.safetensors")
if os.path.exists(qt_sidecar):
    print(f"[4/5] Loading trained audio.q_tokens from sidecar {qt_sidecar}")
    qt_state = load_file(qt_sidecar)
    if "audio.q_tokens" in qt_state:
        with torch.no_grad():
            model.audio.q_tokens.data.copy_(qt_state["audio.q_tokens"].to(model.audio.q_tokens.dtype))
        qt_loaded = True
        print(f"  norm = {float(torch.linalg.norm(model.audio.q_tokens.float())):.6f}")
if not qt_loaded:
    step = os.path.basename(LORA).split("-")[-1]
    ds_path = os.path.join(LORA, f"global_step{step}", "mp_rank_00_model_states.pt")
    if os.path.exists(ds_path):
        print(f"[4/5] Loading trained audio.q_tokens from DeepSpeed state {ds_path}")
        sd = torch.load(ds_path, map_location="cpu", weights_only=False)
        key = "base_model.model.audio.q_tokens"
        if "module" in sd and key in sd["module"]:
            with torch.no_grad():
                model.audio.q_tokens.data.copy_(sd["module"][key].to(model.audio.q_tokens.dtype))
            qt_loaded = True
            print(f"  norm = {float(torch.linalg.norm(model.audio.q_tokens.float())):.6f}")
        del sd
if not qt_loaded:
    print(f"[4/5] [WARN] No trained audio.q_tokens found; merged model will use BASE q_tokens.")

print(f"[5/5] Saving merged → {OUT}")
...
# ---- (2) Build 2 shards from puvalor_test_gout_hinted.json ----
python3 - <<PYEOF
import json, os
...
# ---- (3) Inference on 2 shards (GPU 4,5) ----
GPUS=(4 5)
cd "$BASE_CODE"
...
echo "========== INFERENCE (2 shards parallel) =========="
for r in 0 1; do run_shard $r & done
wait
