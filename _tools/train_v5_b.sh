#!/bin/bash
# ============================================================
# v5_B: PU-VALOR only  /  3+1 tokenization  /  Ordinal regularization (no extra params)
#   변경 점 (이전 3+2 + ord_head 버전 대비):
#     - 토큰화: 3+2 → 3+1  (fps 상 소수점 둘째 자리는 표현 불가능)
#     - Ordinal loss: 별도 ord_head 학습 제거. lm_head 의 time-token row 만 활용한
#                      distance-weighted NLL (label smoothing 의 ordinal 버전).
#                      loss = ce_loss + λ * ord_loss,  λ = 0.1
#     - GPU: 0,1,2,3 (4 GPUs, effective_batch=8 via grad_accum=2)
# ============================================================
set -eo pipefail
...
# output_hidden_states=True 가 학습 중 켜져서 메모리 압박이 있음.
...
OUTPUT_DIR=/data0/aix23102/checkpoints_open_aligner/salmonn2plus_v5_b
RUN_NAME=salmonn2plus_v5_b
mkdir -p "$OUTPUT_DIR"
...
echo "[B] training done; running PU-VALOR test eval"
EVAL_GPUS="0,1,2,3" bash /home/aix23102/audiolm/vS2_eunji/_tools/eval_v5_puvalor.sh \
    "$OUTPUT_DIR" "3_1" "v5_b"
