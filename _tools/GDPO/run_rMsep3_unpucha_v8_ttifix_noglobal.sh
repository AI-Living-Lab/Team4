#!/bin/bash
# ============================================================
# sft_7b_unpucha_v8_rl_rMsep3_unpucha_batch4_noscaling_ttifix_noglobal
#   = run_rMsep3_unpucha_v8_ttifix.sh 와 모두 동일. 유일한 차이: **global reward 제거** (ablation).
#     reward_module → reward_functions_rM_sep3_noglobal (채널 4개: format/count/local/precision)
#     config        → config_sep3_noglobal.yaml (reward_weights 4개)
#   (원본 reward_functions_rM_sep3.py / config_sep3.yaml 무수정 — 별도 파일 신설)
#   ⚠️ GPU 0,1 사용. master_port 29535 (29527 ttifix/29529 nocount/29531 noprecision/29533 nolocal 충돌 회피).
...
RUN=sft_7b_unpucha_v8_rl_rMsep3_unpucha_batch4_noscaling_ttifix_noglobal
...
torchrun --standalone --nproc_per_node=2 --master_port=29535 \
  ...
  --config       _tools/GDPO/config_sep3_noglobal.yaml \
  ...
  --reward_module reward_functions_rM_sep3_noglobal \
  --tti_mode on \
  --output_dir "$OUT" \
  --run_name "$RUN" \
