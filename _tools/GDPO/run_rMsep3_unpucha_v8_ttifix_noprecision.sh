#!/bin/bash
# ============================================================
# sft_7b_unpucha_v8_rl_rMsep3_unpucha_batch4_noscaling_ttifix_noprecision
#   = run_rMsep3_unpucha_v8_ttifix.sh 와 모두 동일. 유일한 차이: **precision reward 제거** (ablation).
#     reward_module → reward_functions_rM_sep3_noprecision (채널 4개: format/count/global/local)
#     config        → config_sep3_noprecision.yaml (reward_weights 4개)
#   (원본 reward_functions_rM_sep3.py / config_sep3.yaml 무수정 — 별도 파일 신설)
#   ⚠️ GPU 0,1 사용 (2,3 은 ttifix 런 점유). master_port 29531 (29527 ttifix 충돌 회피).
...
RUN=sft_7b_unpucha_v8_rl_rMsep3_unpucha_batch4_noscaling_ttifix_noprecision
OUT=$CKPT_DIR/gdpo/$RUN
mkdir -p "$OUT"

# grad_accum=2 → video 재디코드 캐시가 있는 gdpo_trainer_batch.py 사용 (host RAM OOM 방지)
torchrun --standalone --nproc_per_node=2 --master_port=29531 \
  _tools/GDPO/gdpo_trainer_batch.py \
  --config       _tools/GDPO/config_sep3_noprecision.yaml \
  ...
  --reward_module reward_functions_rM_sep3_noprecision \
  --tti_mode on \
  --output_dir "$OUT" \
  --run_name "$RUN" \
