...
class VideoSALMONN2ForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
    ...
    def __init__(self, config, **audio_config):
        ...
        ma = getattr(config, "model_args", {}) or {}
        # model_args might be a dict already
        self.temporal_supervised = bool(ma.get("temporal_supervised", False)) if isinstance(ma, dict) else bool(getattr(ma, "temporal_supervised", False))
        self.temporal_num_bins = int(ma.get("temporal_num_bins", 200)) if isinstance(ma, dict) else int(getattr(ma, "temporal_num_bins", 200))
        self.temporal_loss_weight = float(ma.get("temporal_loss_weight", 1.0)) if isinstance(ma, dict) else float(getattr(ma, "temporal_loss_weight", 1.0))

        if self.temporal_supervised:
            self.temporal_start_head = nn.Linear(config.hidden_size, self.temporal_num_bins, bias=True)
            self.temporal_end_head = nn.Linear(config.hidden_size, self.temporal_num_bins, bias=True)
    ...
    def encode_images(self, images, video_idx_in_batch=[], split_sizes=None):
        ...
        with torch.no_grad():
            for i in range(0, images.shape[0], chunk_size):
                ...
                outs.append(vision_tower(chunk))
        image_features = torch.cat(outs, dim=0)

        # split_sizes 처리 (기존 그대로)
        if split_sizes is None:
            split_sizes = [1 for _ in range(images.shape[0])]

        per_image_features = torch.split(image_features, split_sizes, dim=0)
        all_image_features = []
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            # ============ CHK4: MM INSERT CHECK (rank0 once) ============
            import torch.distributed as dist
            if (not dist.is_available()) or (not dist.is_initialized()) or dist.get_rank() == 0:
                if not hasattr(self, "_dbg_mm_once"):
                    self._dbg_mm_once = True
                    print("[MM][CHK4] count(-200) =", int((cur_input_ids == IMAGE_TOKEN_INDEX).sum().item()))
                    print("[MM][CHK4] modalities =", modalities)
                    if images is None:
                        print("[MM][CHK4] images is None")
                    else:
                        if isinstance(images, (list, tuple)) and len(images) > 0 and torch.is_tensor(images[0]):
                            print("[MM][CHK4] images[0] shape/dtype/device =", tuple(images[0].shape), images[0].dtype, images[0].device)
                        elif torch.is_tensor(images):
                            print("[MM][CHK4] images tensor shape/dtype/device =", tuple(images.shape), images.dtype, images.device)
                        else:
                            print("[MM][CHK4] images type =", type(images))
            # ============================================================
        ...
        if modality_max_length is None or modality_max_length == "None":
            ...
            if isinstance(modality_max_length, str):
                s = modality_max_length.strip()
                if "," in s and not (s.startswith("[") or s.startswith("(")):
                    modality_max_length = [int(x.strip()) for x in s.split(",") if x.strip() != ""]
                else:
                    modality_max_length = ast.literal_eval(s)

            if isinstance(modality_max_length, int):
                modality_max_length = [modality_max_length] * 3

            if not (isinstance(modality_max_length, (list, tuple)) and len(modality_max_length) == 3):
                raise ValueError(f"modality_max_length must be int or length-3 list/tuple, got: {modality_max_length}")

            modality_max_length_dict = {
                "image": int(modality_max_length[0]),
                "text": int(modality_max_length[1]),
                "video": int(modality_max_length[2]),
                "audio": int(modality_max_length[2]),
                "audio-video": int(modality_max_length[2]),
            }
            ...
            for x, modality in zip(new_input_embeds, modalities):
                max_len = modality_max_length_dict.get(modality, default_len)
                sliced_input_embeds.append(x[:max_len])

            for x, modality in zip(new_labels, modalities):
                max_len = modality_max_length_dict.get(modality, default_len)
                sliced_labels.append(x[:max_len])

            new_input_embeds = sliced_input_embeds
            new_labels = sliced_labels
    ...
    def forward(
        ...
        **kwargs,
        ...
        if dpo_forward:
            ...
            need_hs = bool(self.temporal_supervised and (gt_timestamps is not None))
            outputs = super().forward(
                ...
                use_cache=use_cache,
                output_attentions=output_attentions,
                output_hidden_states=(True if need_hs else output_hidden_states),
                return_dict=(True if (return_dict is None) else return_dict),
            )

            # Add supervised temporal localization loss (time-bin CE) if enabled
            if self.temporal_supervised and (gt_timestamps is not None):
                # gt_timestamps: list length B, each is [start_sec, end_sec] or None
                # real_time: list length B, total duration in seconds
                try:
                    hs = outputs.hidden_states[-1]  # [B, L, H]
                    B, L, H = hs.shape

                    # pick last non-pad token representation per sample
                    if attention_mask is None:
                        # fallback: last token
                        pooled = hs[:, -1, :]
                    else:
                        # attention_mask in this code is float dtype sometimes; treat >0 as valid
                        am = (attention_mask > 0).to(torch.long)
                        last_idx = (am.sum(dim=1) - 1).clamp(min=0)  # [B]
                        pooled = hs[torch.arange(B, device=hs.device), last_idx, :]  # [B, H]

                    start_logits = self.temporal_start_head(pooled)  # [B, N]
                    end_logits = self.temporal_end_head(pooled)      # [B, N]

                    # build bin labels
                    start_bins = []
                    end_bins = []
                    for i in range(B):
                        gt = gt_timestamps[i] if isinstance(gt_timestamps, (list, tuple)) else None
                        T = float(real_time[i]) if isinstance(real_time, (list, tuple)) else None
                        if (gt is None) or (T is None) or (T <= 0):
                            start_bins.append(-100)
                            end_bins.append(-100)
                            continue
                        s = float(gt[0]); e = float(gt[1])
                        s = max(0.0, min(s, T))
                        e = max(0.0, min(e, T))
                        # normalize to [0,1]
                        s_n = s / T
                        e_n = e / T
                        N = self.temporal_num_bins
                        sb = int(round(s_n * (N - 1)))
                        eb = int(round(e_n * (N - 1)))
                        sb = max(0, min(sb, N - 1))
                        eb = max(0, min(eb, N - 1))
                        start_bins.append(sb)
                        end_bins.append(eb)

                    start_bins = torch.tensor(start_bins, device=hs.device, dtype=torch.long)
                    end_bins = torch.tensor(end_bins, device=hs.device, dtype=torch.long)

                    # ignore samples without GT (-100)
                    ce = nn.CrossEntropyLoss(ignore_index=-100)
                    t_loss = ce(start_logits, start_bins) + ce(end_logits, end_bins)
                    outputs.loss = outputs.loss + (self.temporal_loss_weight * t_loss)

                except Exception:
                    # keep training even if temporal loss fails
                    pass
