...
# -*- coding: utf-8 -*-
"""
reward_functions_rM_sep3_noprecision.py
  reward_functions_rM_sep3 의 ablation — **precision 채널만 제거**한 버전.
  (reward w/o precision ablation: temporal precision(overlap/pred_len) 신호를 뺐을 때의 영향 측정용)

  채널 4개 (sep3 에서 precision 제외, 전부 [0,1]):
  ...
  ⚠️ 원본 reward_functions_rM_sep3.py / reward_functions_rM_sep.py 는 **수정하지 않고 import 만**.
     precision 를 뺐으므로 reward_weights 는 4개 [format, count, global, local] 로 매칭
     (config_sep3_noprecision.yaml). 채널명=로깅명(wandb rewards/<name> + 데이터셋별 rewards/<name>/<tag>).
"""
...
from reward_functions_rM_sep3 import count_matching_reward   # 세그먼트 개수 매칭 (구 len)


# 채널 스펙 — (name, fn, needs_gt). precision 제거 → 4채널. 가중치 순서 [format, count, global, local].
